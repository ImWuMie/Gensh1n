...
public enum Formatting {
    ...
    private static final Map<String, Formatting> FORMATTING_BY_NAME = Arrays.stream(values()).collect(Collectors.toMap((format) -> cleanName(format.name), (format) -> format));
    ...
    Formatting(String name, char code, int id, Integer color) {
        this(name, code, false, id, color);
    }

    Formatting(String name, char code, boolean isFormat) {
        this(name, code, isFormat, -1, null);
    }

    Formatting(String name, char code, boolean isFormat, int id, Integer color) {
        this.name = name;
        this.code = code;
    ...
    public static Formatting getById(int id) {
        if (id < 0) {
            return RESET;
        } else {
            for (Formatting chatformatting : values()) {
