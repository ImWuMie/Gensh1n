...
import dev.undefinedteam.modernui.mc.fabric.MuiFabricApi;
...
public abstract class MuiModApi {
    ...
    public interface OnScreenChangeListener {
        ...
        /**
         * Called when {@link MinecraftClient#setScreen(Screen)} is called, and after Forge's
         * event is fired.
         *
    ...
    public interface OnWindowResizeListener {
        ...
        /**
         * Invoked at the beginning of {@link MinecraftClient}.
     ...
     * and {@link MinecraftClient#setScreen(Screen)}.
     *
     ...
     * To start the lifecycle of the fragment, use {@link MinecraftClient#setScreen(Screen)}.
     ...
     */
    @Nonnull
    public abstract <T extends ScreenHandler, U extends Screen & ScreenHandlerProvider<T> & MuiScreen>
    ...
    public static int calcGuiScales() {
        return calcGuiScales(MinecraftClient.getInstance().getWindow());
    }

    public static int calcGuiScales(@Nonnull Window window) {
        return calcGuiScales(window.getFramebufferWidth(), window.getFramebufferHeight());
    }
    ...
    public static int calcGuiScales(int framebufferWidth, int framebufferHeight) {
        double w = framebufferWidth / 16.0;
        double h = framebufferHeight / 9.0;
        ...
        int auto;
        if (min >= 2) {
            double step = base > 216 ? 42.0 : base > 120 ? 36.0 : 30.0;
            int i = (int) (base / step);
            int j = (int) (Math.max(w, h) / step);
            double v1 = base / (i * 30.0);
            if (v1 > 42 / 30.0 || j > i) {
    ...
    public static int offsetByGrapheme(String value, int cursor, int dir) {
        ...
        int offset = Util.moveCursor(value, cursor, dir);
    ...
    /**
     * Maps ASCII to Formatting, including all cases.
     */
    private static final Formatting[] FORMATTING_TABLE = new Formatting[128];

    static {
        for (Formatting f : Formatting.values()) {
            FORMATTING_TABLE[f.getCode()] = f;
            FORMATTING_TABLE[Character.toUpperCase(f.getCode())] = f;
        }
    }

    /**
     * Get the {@link Formatting} by the given formatting code. Vanilla's method is
     * overwritten by this, see {@link Formatting}.
     * <p>
     * Vanilla would create a new String from the char, call String.toLowerCase() and
     * String.charAt(0), search this char with a clone of Formatting values. However,
    ...
    public abstract ShaderProgram makeShaderInstance(ResourceFactory resourceProvider,
                                                     Identifier resourceLocation,
                                                     VertexFormat vertexFormat) throws IOException;

    public abstract boolean isKeyBindingMatches(KeyBinding keyMapping, InputUtil.Key key);
    ...
    /**
     * Registers a callback to be called when {@link MinecraftClient#setScreen(Screen)} is called.
     *
     * @param listener the listener to register
    ...
    /**
     * Registers a callback to be invoked at the beginning of {@link MinecraftClient}.
     *
     * @param listener the listener to register
