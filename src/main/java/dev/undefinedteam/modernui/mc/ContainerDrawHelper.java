...
public final class ContainerDrawHelper {
    ...
    private final ItemRenderer mRenderer = MinecraftClient.getInstance().getItemRenderer();
    private final TextureManager mTextureManager = MinecraftClient.getInstance().getTextureManager();
    ...
    private static class DrawItem implements CustomDrawable.DrawHandler {
        ...
        public void draw(DirectContext dContext, DrawableInfo info) {
            ...
            BakedModel model = minecraft.getItemRenderer().getModel(item, minecraft.world, minecraft.player, seed);
            boolean light2D = !model.isSideLit();
            if (light2D) {
            ...
            MatrixStack localTransform = new MatrixStack();
            localTransform.multiplyPositionMatrix(pose);
            VertexConsumerProvider.Immediate bufferSource = minecraft.getBufferBuilders().getEntityVertexConsumers();
            minecraft.getItemRenderer().renderItem(item, ModelTransformationMode.GUI, false, localTransform, bufferSource,
