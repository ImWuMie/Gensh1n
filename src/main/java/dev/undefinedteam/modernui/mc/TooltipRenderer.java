...
public final class TooltipRenderer {
    ...
    public static volatile float sBorderWidth = 4 / 3.0f;
    ...
    void computeWorkingColor(@Nonnull ItemStack item) {
        if (sAdaptiveColors && !item.isEmpty()) {
            if (sRoundedShapes && (item.isOf(Items.DRAGON_EGG) ||
                    item.isOf(Items.MOJANG_BANNER_PATTERN))) {
                mUseSpectrum = true;
                return;
            }
            ...
            TextVisitFactory.visitFormatted(
                    ...
                    (i, style, ch) -> {
                        ...
                        if (textColor != null) {
                            return colors.add(textColor.getRgb() & 0xFFFFFF) &&
                                    colors.size() >= 16;
                        }
                        return false;
                    }
            ...
            if (!colors.isEmpty()) {
                ...
                if (!hsvColors.isEmpty()) {
                    ...
                    if (size > 2) {
                        ...
                        c4 = adjustColor(hsv, false, true, false, item.hasEnchantments());
                    } else {
                        // we have one color...
                        float[] hsv = hsvColors.get(0);
                        boolean mag = item.hasEnchantments();
    ...
    void chooseBorderColor(int corner, Uniform uniform) {
        int color = chooseBorderColor(corner);
        int a = (color >>> 24);
        int r = ((color >> 16) & 0xff);
        int g = ((color >> 8) & 0xff);
        int b = (color & 0xff);
        uniform.set(r / 255.0f, g / 255.0f, b / 255.0f, a / 255.0f);
    }

    public void drawTooltip(@Nonnull ItemStack itemStack, @Nonnull DrawContext gr,
                            @Nonnull List<TooltipComponent> list, int mouseX, int mouseY,
                            @Nonnull TextRenderer font, int screenWidth, int screenHeight,
                            float partialX, float partialY, @Nullable TooltipPositioner positioner) {
        mDraw = true;
        ...
        if (list.size() == 1) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (i == 0 && !itemStack.isEmpty() &&
                        component instanceof OrderedTextTooltipComponent) {
                    titleGap = true;
        ...
        if (positioner != null) {
            var pos = positioner.getPosition(screenWidth, screenHeight,
                    mouseX, mouseY,
        ...
        gr.getMatrices().push();
        // because of the order of draw calls, we actually don't need z-shifting
        gr.getMatrices().translate(0, -mScroll, 400);
        final Matrix4f pose = gr.getMatrices().peek().getPositionMatrix();
        ...
        final VertexConsumerProvider.Immediate source = gr.getVertexConsumers();
        gr.getMatrices().translate(partialX, partialY, 0);
        for (int i = 0; i < list.size(); i++) {
            TooltipComponent component = list.get(i);
            if (titleGap && i == 0 && sCenterTitle) {
                component.drawText(font, drawX + (tooltipWidth - component.getWidth(font)) / 2, drawY, pose, source);
            } else if (mLayoutRTL) {
                component.drawText(font, drawX + tooltipWidth - component.getWidth(font), drawY, pose, source);
            } else {
                component.drawText(font, drawX, drawY, pose, source);
            }
            if (titleGap && i == 0) {
                drawY += TITLE_GAP;
            }
        ...
        for (int i = 0; i < list.size(); i++) {
            TooltipComponent component = list.get(i);
            if (mLayoutRTL) {
                component.drawItems(font, drawX + tooltipWidth - component.getWidth(font), drawY, gr);
            } else {
                component.drawItems(font, drawX, drawY, gr);
            }
            if (titleGap && i == 0) {
                drawY += TITLE_GAP;
            }
    ...
    private void drawRoundedBackground(@Nonnull DrawContext gr, Matrix4f pose,
        ...
        ShaderProgram shader = TooltipRenderType.getShaderTooltip();
        shader.getUniformOrDefault("u_PushData0")
                .set(sizeX, sizeY, sCornerRadius, sBorderWidth / 2.0f);
        ...
        shader.getUniformOrDefault("u_PushData1")
                .set(sShadowAlpha, 4.0f / shadowRadius, (sFillColor[0] >>> 24) / 255.0f, rainbowOffset);
        if (rainbowOffset == 0) {
            chooseBorderColor(0, shader.getUniformOrDefault("u_PushData2"));
            chooseBorderColor(1, shader.getUniformOrDefault("u_PushData3"));
            chooseBorderColor(3, shader.getUniformOrDefault("u_PushData4"));
            chooseBorderColor(2, shader.getUniformOrDefault("u_PushData5"));
        }

        var buffer = gr.getVertexConsumers().getBuffer(TooltipRenderType.tooltip());

        // we expect local coordinates, concat pose with model view
        RenderSystem.getModelViewStack().push();
        RenderSystem.getModelViewStack().multiplyPositionMatrix(pose);
        ...
        buffer.vertex(extentX, extentY, 0);
        buffer.vertex(extentX, -extentY, 0);
        buffer.vertex(-extentX, -extentY, 0);
        buffer.vertex(-extentX, extentY, 0);
    ...
    private void drawVanillaBackground(@Nonnull DrawContext gr, Matrix4f pose,
                                       float tooltipX, float tooltipY,
                                       int tooltipWidth, int tooltipHeight,
                                       boolean titleGap, int titleBreakHeight) {
    ...
    private static void fillGrad(DrawContext gr, Matrix4f pose,
                                 float left, float top, float right, float bottom,
                                 int colorUL, int colorUR, int colorLR, int colorLL) {
        var buffer = gr.getVertexConsumers().getBuffer(RenderLayer.getGui());
