...
public class TextLayout {
    ...
    public static final TextLayout EMPTY = new TextLayout(new char[0], new int[0], new float[0],
        null, new Font[0], new float[0], new int[0], new int[]{0}, 0.0f, 0, false, false, 2, ~0) {
        @Nonnull
        @Override
        TextLayout get() {
        ...
        @Override
        public void drawTextOutline(@NotNull Matrix4f matrix, @NotNull VertexConsumerProvider source, float x, float top, int r, int g, int b, int a, int packedLight) {
        }

        @Override
        public float drawText(@NotNull Matrix4f matrix, @NotNull VertexConsumerProvider source, float x, float top, int r, int g, int b, int a, boolean isShadow, int preferredMode, boolean polygonOffset, int bgColor, int packedLight) {
            return 0;
        }
    };
    ...
    private BakedGlyph[] prepareGlyphs(int resLevel, int fontSize) {
        ...
        for (int i = 0; i < glyphs.length; i++) {
            ...
            if (glyphs[i] != null && glyphs[i].height > mMaxHeight) {
                mMaxHeight = glyphs[i].height;
            }
        }
        return glyphs;
    }
    ...
    @Nonnull
    private BakedGlyph[] getGlyphsUniformScale(float density, int resLevel) {
    ...
    public float drawText(@Nonnull final Matrix4f matrix,
                          @Nonnull final VertexConsumerProvider source,
        ...
        if (preferredMode == TextRenderType.MODE_SDF_FILL) {
            ...
            glyphs = getGlyphsUniformScale(density, mCreatedResLevel);
        ...
        final float baseline = top + this.computeFontSize(mCreatedResLevel) - this.mFontSize / 8.0f;
        ...
        for (int i = 0, e = glyphs.length; i < e; i++) {
            ...
            int ascent = 0;
            TextRenderer.TextLayerType compatDisplayMode = null;
            if ((bits & CharacterStyle.ANY_BITMAP_REPLACEMENT) != 0) {
                float scaleFactor = TextLayoutEngine.BITMAP_SCALE;
                if ((bits & CharacterStyle.COLOR_EMOJI_REPLACEMENT) != 0) {
                    if (isShadow) {
                        continue;
                    }

                    scaleFactor *= (this.mFontSize / 8) * 0.125f;
                }
                ...
                if (polygonOffset) {
                    compatDisplayMode = TextRenderer.TextLayerType.POLYGON_OFFSET;
                }
                if (getFont(i) instanceof BitmapFont bitmapFont) {
                    ...
                    ascent = (int) (computeFontSize(mCreatedResLevel) - this.mFontSize / 8.0f);
                }
                ...
                if (obfuscated && getFont(i) instanceof BitmapFont bitmapFont) {
                    ...
                    float scaleFactor = 1.0f / TextLayoutEngine.BITMAP_SCALE;
                    rx += x + positions[i << 1] + (float) glyph.x * scaleFactor;
                    ry = baseline + positions[i << 1 | 1] + (float) glyph.y * scaleFactor;
                    if (isShadow) {
                        // bitmap font shadow offset is always 1 pixel
     ...
     * @param packedLight see {@link net.minecraft.client.render.LightmapTextureManager}
     */
    @SuppressWarnings("UnnecessaryLocalVariable")
    public void drawTextOutline(@Nonnull Matrix4f matrix,
                                @Nonnull VertexConsumerProvider source,
                                final float x, final float top,
                                int r, int g, int b, int a,
                                int packedLight) {
        ...
        final float baseline = top + computeFontSize((int) resLevel) - this.mFontSize / 8.0f;
        ...
        for (int i = 0, e = glyphs.length; i < e; i++) {
            ...
            builder.vertex(matrix, rx - sBloat, ry - sBloat, 0.001f)
                .color(r, g, b, a)
                .texture(glyph.u1 - uBloat, glyph.v1 - vBloat)
                .light(packedLight)
                .next();
            builder.vertex(matrix, rx - sBloat, ry + h + sBloat, 0.001f)
                .color(r, g, b, a)
                .texture(glyph.u1 - uBloat, glyph.v2 + vBloat)
                .light(packedLight)
                .next();
            builder.vertex(matrix, rx + w + sBloat, ry + h + sBloat, 0)
                .color(r, g, b, a)
                .texture(glyph.u2 + uBloat, glyph.v2 + vBloat)
                .light(packedLight)
                .next();
            builder.vertex(matrix, rx + w + sBloat, ry - sBloat, 0)
                .color(r, g, b, a)
                .texture(glyph.u2 + uBloat, glyph.v1 - vBloat)
                .light(packedLight)
                .next();
        }
    }
