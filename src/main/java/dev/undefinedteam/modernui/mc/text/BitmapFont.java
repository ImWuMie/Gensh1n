 ...
 *
 * @author BloCamLimb
 * @see net.minecraft.client.font.BitmapFont
 * @since 3.6
 */
public class BitmapFont implements Font, AutoCloseable {
    ...
    private BitmapFont(Identifier name, Bitmap bitmap,
                       ...
                       int height, int ascent,float fontSize) {
        mName = name;
        mBitmap = bitmap;
        mAscent = ascent;
    ...
    @Nonnull
    public static BitmapFont create(net.minecraft.client.font.BitmapFont.Loader definition, ResourceManager manager,float size) {
        ...
        var location = file.withPrefixedPath("textures/");
        try (InputStream stream = manager.open(location)) {
            ...
            return new BitmapFont(file, bitmap, grid, rows, cols, height, ascent,size);
        } catch (Exception e) {
    ...
    public float doComplexLayout(char[] buf,
        ...
        float scaleUp = (int) (paint.getFontSize() / mFontSize + 0.5);
    ...
    public static class Glyph extends BakedGlyph implements net.minecraft.client.font.Glyph {
        ...
        @Nonnull
        @Override
        public GlyphRenderer bake(@Nonnull Function<RenderableGlyph, GlyphRenderer> function) {
