...
import dev.undefinedteam.modernui.mc.FontResourceManager;
import dev.undefinedteam.modernui.mc.ModernUIClient;
import dev.undefinedteam.gensh1n.mixins.modernui.text.MixinGameRenderer;
...
/**
 * Modern UI text engine for MinecraftClient. This class performs Unicode text layout (and measurement),
 ...
 * This class also handles font and emoji resources used in MinecraftClient.
 * <p>
...
public class TextLayoutEngine extends FontResourceManager
     ...
     */
    private Map<MutableText, TextLayout> mComponentCache = new HashMap<>();
     ...
     */
    private final Map<Float, TextLayoutProcessor> mProcessorPool = Collections.synchronizedMap(new HashMap<>());

    private record FontStrikeDesc(Font font, int fontSize) {
    ...
    public static final int MIN_PIXEL_DENSITY_FOR_SDF = 1;
    ...
    public TextLayoutEngine() {
        ...
        mTextRenderer = new ModernTextRenderer(this, 10);
    ...
    public void reload() {
        ...
        if (opts != null) { // this can be null on Fabric, because this class loads too early
            mForceUnicodeFont = opts.getForceUnicodeFont().getValue();
        }
        ...
        mFontCollections.putIfAbsent(new Identifier(SANS_SERIF.getPath()),
            Typeface.SANS_SERIF);
        mFontCollections.putIfAbsent(new Identifier(SERIF.getPath()),
            Typeface.SERIF);
        mFontCollections.putIfAbsent(new Identifier(MONOSPACED.getPath()),
            Typeface.MONOSPACED);
        ...
        if (sDefaultFontBehavior == DEFAULT_FONT_BEHAVIOR_IGNORE_ALL || // exclude everything
            ...
            mFontCollections.put(MinecraftClient.DEFAULT_FONT_ID, ModernUI.getSelectedTypeface());
        } else {
            ...
            mFontCollections.put(MinecraftClient.DEFAULT_FONT_ID,
    ...
    @Nonnull
    @Override
    public CompletableFuture<Void> reload(@Nonnull Synchronizer preparationBarrier,
                                          @Nonnull ResourceManager resourceManager,
                                          @Nonnull Profiler preparationProfiler,
                                          @Nonnull Profiler reloadProfiler,
    ...
    private void applyResources(@Nonnull LoadResults results) {
        ...
        mRawDefaultFontCollection = mFontCollections.get(MinecraftClient.DEFAULT_FONT_ID);
        if (mRawDefaultFontCollection == null) {
    ...
    public void onFontRegistered(@Nonnull FontFamily f) {
        ...
        try {
            ...
            if (mRegisteredFonts.putIfAbsent(location, fc) == null) {
                ...
                mRegisteredFonts.putIfAbsent(new Identifier(newName), fc);
            }
        } catch (Exception e) {
    ...
    private static boolean isUnicodeFont(@Nonnull Identifier name) {
        if (name.equals(MinecraftClient.UNICODE_FONT_ID)) {
            return true;
        }
    ...
    private static void loadFonts(@Nonnull ResourceManager resources, @Nonnull LoadResults results) {
        ...
        for (var entry : resources.findAllResources("font",
            ...
            for (var resource : entry.getValue()) {
                try (var reader = resource.getReader()) {
                    var providers = JsonHelper.getArray(Objects.requireNonNull(
                        JsonHelper.deserialize(gson, reader, JsonObject.class)), "providers");
                    for (int i = 0; i < providers.size(); i++) {
                        var metadata = JsonHelper.asObject(
                            providers.get(i), "providers[" + i + "]");
                        var definition = FontLoader.CODEC.parse(
                            JsonOps.INSTANCE,
                            metadata
                        ).get().orThrow();
                        loadSingleFont(resources, name, bundle,
                            resource.getResourcePackName(), i, metadata, definition);
                    }
                    LOGGER.info(MARKER, "Loaded raw font '{}' in pack: '{}'",
                        name, resource.getResourcePackName());
                } catch (Exception e) {
                    LOGGER.warn(MARKER, "Failed to load font '{}' in pack: '{}'",
                        name, resource.getResourcePackName(), e);
                }
            }
        ...
        final var sorter = new DependencyTracker<Identifier, RawFontBundle>();
        for (var bundle : bundles) {
            sorter.add(bundle.name, bundle);
        }
        final var map = new HashMap<Identifier, FontCollection>();
        sorter.traverse((name, bundle) -> {
            if (isUnicodeFont(name)) {
                return;
            }
    ...
    private static void loadSingleFont(@Nonnull ResourceManager resources,
                                       ...
                                       JsonObject metadata,
                                       @Nonnull FontLoader definition) {
        switch (definition.getType()) {
            case BITMAP -> {
                for (float size : TextLayoutEngine.getInstance().mProcessorPool.keySet()) {
                    var bitmapFont = BitmapFont.create((net.minecraft.client.font.BitmapFont.Loader) definition, resources, size);
                    bundle.families.add(
            ...
            case TTF -> {
                var ttf = (TrueTypeFontLoader) definition;
            ...
            case SPACE -> {
                for (float size : TextLayoutEngine.getInstance().mProcessorPool.keySet()) {
                    var spaceFont = SpaceFont.create(name, (net.minecraft.client.font.SpaceFont.Loader) definition, size);
                    bundle.families.add(
            ...
            case REFERENCE -> {
                Identifier reference = ((ReferenceFont) definition).id();
     ...
     */
    @Nonnull
    public TextLayout lookupVanillaLayout(@Nonnull String text, float size) {
        return lookupVanillaLayout(text, Style.EMPTY, 0, size);
    }
     ...
     */
    @Nonnull
    public TextLayout lookupVanillaLayout(@Nonnull String text, @Nonnull Style style, float size) {
        return lookupVanillaLayout(text, style, 0, size);
    }

    private TextLayoutProcessor lookupLayoutProcessor(float fontSize) {
        TextLayoutProcessor proc = this.mProcessorPool.getOrDefault(fontSize, null);
        if (proc == null || proc.mFontSize != fontSize) {
            proc = new TextLayoutProcessor(this, fontSize);
            this.mProcessorPool.put(fontSize, proc);
            LOGGER.info("New processor(S:){}s created", fontSize);
        }
        return proc;
    }
    ...
    public TextLayout lookupVanillaLayout(@Nonnull String text, @Nonnull Style style,
                                          int computeFlags, float fontSize) {
        if (text.isEmpty()) {
            return TextLayout.EMPTY;
        }
        if (!RenderSystem.isOnRenderThread()) {
            if (sAllowAsyncLayout) {
                return lookupLayoutProcessor(fontSize).createVanillaLayout(text, style, mResLevel, computeFlags);
            } else {
                return MinecraftClient.getInstance().submit(
                        () -> lookupVanillaLayout(text, style, computeFlags, fontSize)
                    )
                    .join();
            }
        }
        ...
        int nowFlags = 0;
        if (layout == null ||
            ((nowFlags = layout.mComputedFlags) & computeFlags) != computeFlags || layout.mFontSize != fontSize) {
            layout = lookupLayoutProcessor(fontSize).createVanillaLayout(text, style, mResLevel,
     ...
     */
    @Nonnull
    public TextLayout lookupFormattedLayout(@Nonnull StringVisitable text, float fontSize) {
        return lookupFormattedLayout(text, Style.EMPTY, 0, fontSize);
    }
     ...
     */
    @Nonnull
    public TextLayout lookupFormattedLayout(@Nonnull StringVisitable text, @Nonnull Style style, float fontSize) {
        return lookupFormattedLayout(text, style, 0, fontSize);
    }
     ...
     */
    @Nonnull
    public TextLayout lookupFormattedLayout(@Nonnull StringVisitable text, @Nonnull Style style,
                                            int computeFlags, float fontSize) {
        ...
        if (!RenderSystem.isOnRenderThread()) {
            if (sAllowAsyncLayout) {
                return lookupLayoutProcessor(fontSize).createTextLayout(text, style, mResLevel, computeFlags);
            } else {
                return MinecraftClient.getInstance().submit(
        ...
        if (style.isEmpty() && sUseComponentCache &&
            text instanceof MutableText component) {
            layout = mComponentCache.get(component);
            if (layout == null ||
                ((nowFlags = layout.mComputedFlags) & computeFlags) != computeFlags || layout.mFontSize != fontSize) {
                layout = lookupLayoutProcessor(fontSize).createTextLayout(text, Style.EMPTY, mResLevel,
                    nowFlags | computeFlags);
            ...
            if (layout == null ||
                ((nowFlags = layout.mComputedFlags) & computeFlags) != computeFlags || layout.mFontSize != fontSize) {
                layout = lookupLayoutProcessor(fontSize).createTextLayout(text, style, mResLevel,
                    nowFlags | computeFlags);
     ...
     */
    @Nonnull
    public TextLayout lookupFormattedLayout(@Nonnull OrderedText sequence, float fontSize) {
        return lookupFormattedLayout(sequence, 0, fontSize);
    }
     ...
     */
    @Nonnull
    public TextLayout lookupFormattedLayout(@Nonnull OrderedText sequence,
                                            int computeFlags, float fontSize) {
        if (sequence == OrderedText.EMPTY) {
            return TextLayout.EMPTY;
        }
        if (!RenderSystem.isOnRenderThread()) {
            if (sAllowAsyncLayout) {
                return lookupLayoutProcessor(fontSize).createSequenceLayout(sequence, mResLevel, computeFlags);
            } else {
                return MinecraftClient.getInstance().submit(
        ...
        if (sequence instanceof FormattedTextWrapper) {
            StringVisitable text = ((FormattedTextWrapper) sequence).mText;
            if (text == ScreenTexts.EMPTY || text == StringVisitable.EMPTY) {
                return TextLayout.EMPTY;
            }
            TextLayout layout;
            ...
            if (layout == null ||
                ((nowFlags = layout.mComputedFlags) & computeFlags) != computeFlags || layout.mFontSize != fontSize) {
                layout = lookupLayoutProcessor(fontSize).createSequenceLayout(sequence, mResLevel,
                    nowFlags | computeFlags);
                mFormattedCache.put(mFormattedLayoutKey.copy(), layout);
                return layout;
            }
     ...
     * it not a reordered text, see {@link}.
     ...
     */
    @Deprecated
    public boolean handleSequence(OrderedText sequence, ReorderTextHandler.IConsumer consumer) {
     ...
     */
    @Nonnull
    public FontCollection getFontCollection(@Nonnull Identifier fontName,float fontSize) {
        if (mForceUnicodeFont == Boolean.TRUE &&
            fontName.equals(MinecraftClient.DEFAULT_FONT_ID)) {
            fontName = MinecraftClient.UNICODE_FONT_ID;
        }
        FontCollection fontCollection;
        var parent = this.mFontCollections.get(fontName);
        if (parent != null) {
            List<FontFamily> families = new ArrayList<>(parent.getFamilies());
            families.removeIf(f -> {
                var font = f.getClosestMatch(FontPaint.NORMAL);
                if (font instanceof BitmapFont b && b.mFontSize != fontSize) {
                    return true;
                } else return font instanceof SpaceFont s && s.mFontSize != fontSize;
            });

            fontCollection = new FontCollection(families.toArray(FontFamily[]::new));
        } else fontCollection = ModernUI.getSelectedTypeface();
     ...
     */
    @Nullable
    public FastCharSet lookupFastChars(@Nonnull Font font, int fontSize) {
        if (font == mEmojiFont) {
            // Emojis are supported for obfuscated rendering
            return null;
        }
        return mFastCharMap.computeIfAbsent(
            new FontStrikeDesc(font, fontSize),
            mCacheFastChars
        );
    }
    ...
    private FastCharSet cacheFastChars(@Nonnull FontStrikeDesc desc) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (awtFont != null) {
                ...
                advance = (float) vector.getGlyphPosition(1).getX();
