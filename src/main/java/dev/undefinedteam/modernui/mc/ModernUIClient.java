...
public abstract class ModernUIClient extends ModernUI {
    ...
    public static void loadFonts(String first,
        ...
        if (firstLoad) {
            ...
            var directory = MinecraftClient.getInstance().getResourcePackDir();
                    ...
                    try (var inputStream = resource.getInputStream()) {
                        ...
                        for (var f : families) {
                            ...
                            LOGGER.info(MARKER, "Registered font '{}', location '{}' in pack: '{}'",
                                    f.getFamilyName(), entry.getKey(), resource.getPack());
                        }
                    } catch (Exception e) {
                        LOGGER.error(MARKER, "Failed to register font '{}' in pack: '{}'",
                                entry.getKey(), resource.getPack(), e);
                    }
                }
            }
        }
    ...
    protected Typeface onGetSelectedTypeface() {
        ...
        synchronized (this) {
            ...
            if (mTypeface == null) {
                ...
                MinecraftClient.getInstance().send(() -> LayoutCache.getOrCreate(new char[]{'M'},
    ...
    public InputStream getResourceStream(@Nonnull String namespace, @Nonnull String path) throws IOException {
        return MinecraftClient.getInstance().getResourceManager().open(Identifier.of(namespace, path));
    }
