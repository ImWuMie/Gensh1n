...
public class MusicFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container,
...

        {
            ...
            seekLayout.mSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                ...
                public void onStopTrackingTouch(SeekBar seekBar) {
                    mMusicPlayer.seek(seekBar.getProgress() / 10000.0f);
                    if (mPlaying) {
                        mMusicPlayer.play();
                    }
                }
...

        {
            ...
            volumeBar.mSeekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                ...
                public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                    if (fromUser) {
                        mMusicPlayer.setGain(progress / 10000.0f);
                    }
                }
            });
        }
    ...
    public static class SpectrumDrawable extends Drawable {
        ...
        private void computeParticles(float multiplier, long deltaMillis) {
            float delta = deltaMillis / 1000.0f;
        ...
        public void draw(@NonNull Canvas canvas) {
            ...
            for (int i = 0; i < len; i++) {
                // 2.5e-5f * BPM
                amplitudes[i] = Math.max(0, amplitudes[i] - delta * 2.5e-5f * 180.0f * (amplitudes[i] + 0.03f));
            }
            ...
            float alphaMult = 1.5f + MathUtil.sin(time / 600.0f) / 2;
