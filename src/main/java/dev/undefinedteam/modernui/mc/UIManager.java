...
public abstract class UIManager implements LifecycleOwner {
    ...
    // minecraft window
    protected final net.minecraft.client.util.Window mWindow = minecraft.getWindow();

    private final MatrixStack mEmptyMatrixStack = new MatrixStack();
    ...
    public GLSurfaceCanvas mCanvas;
    public GLDevice mDevice;
    public final Matrix4 mProjectionMatrix = new Matrix4();
        ...
        while (mRunning) {
            try {
                ...
                if (mRunning && ModernUIMod.isDeveloperMode()) {
                    ...
                    minecraft.setCrashReportSupplier(CrashReport.create(e, "Exception on UI thread"));
                }
            }
            break;
        }
    ...
    public void onHoverMove(boolean natural) {
        ...
        float x = (float) (minecraft.mouse.getX() *
            mWindow.getWidth() / mWindow.getFramebufferWidth());
    ...
    public void onScroll(double scrollX, double scrollY) {
        if (mScreen != null) {
            final long now = Core.timeNanos();
            final net.minecraft.client.util.Window window = mWindow;
    ...
    protected void changeRadialBlur() {
        if (minecraft.gameRenderer.getPostProcessor() == null) {
            ...
            if (InputUtil.isKeyPressed(mWindow.getHandle(), GLFW_KEY_RIGHT_SHIFT)) {
                effect = new Identifier("shaders/post/grayscale.json");
            } else {
                effect = new Identifier("shaders/post/radial_blur.json");
            }
    ...
    public void dump() {
        ...
        if (minecraft.world != null) {
            ...
            minecraft.inGameHud.getChatHud().addMessage(Text.literal(str).formatted(Formatting.GRAY));
        }
    ...
    public void dump(@Nonnull PrintWriter pw, boolean fragments) {
        ...
        if (menu != null) {
            ...
            try {
                Identifier name = Registries.SCREEN_HANDLER.getId(menu.getType());
    ...
    public void render() {
        if (mCanvas == null || mNoRender) {
            /*if (mScreen != null) {
                ...
                minecraft.font.draw(mEmptyMatrixStack, error, x, y, 0xFFFF0000);
            }*/
            return;
        }
        ...
        mCanvas.setProjection(mProjectionMatrix.setOrthographic(
            width, height, 0, Window.LAST_SYSTEM_WINDOW * 2 + 1,
            true));
    ...
    private final Runnable mResizeRunnable = () -> mRoot.setFrame(mWindow.getFramebufferWidth(), mWindow.getFramebufferHeight());
    ...
    public void drawExtTooltip(ItemStack itemStack,
        ...
        final net.minecraft.client.util.Window window = mWindow;
        final Mouse mouseHandler = minecraft.mouse;
        final double cursorX = mouseHandler.getX() *
            (double) window.getScaledWidth() / (double) window.getWidth();
        final double cursorY = mouseHandler.getY() *
            (double) window.getScaledHeight() / (double) window.getHeight();
        ...
        if (TooltipRenderer.sExactPositioning &&
            positioner instanceof HoveredTooltipPositioner) {
            positioner = null;
        }
        if (x == mouseX && y == mouseY && positioner == null &&
            isIdentity(graphics.getMatrices().peek().getPositionMatrix())) {
    ...
    public static void destroy() {
        ...
        LOGGER.debug(MARKER, "Quiting UI");
        if (sInstance != null) {
            ...
            if (screen != null && event.getAction() == KeyEvent.ACTION_DOWN) {
                ...
                if (screen.getCallback() != null) {
                    ...
                    if (event.getKeyCode() == KeyEvent.KEY_ESCAPE) {
                        ...
                        InputUtil.Key key = InputUtil.fromKeyCode(event.getKeyCode(), event.getScanCode());
                        back = MuiModApi.get().isKeyBindingMatches(minecraft.options.inventoryKey, key);
                    }
                } else {
        ...
        private void flushDrawCommands(GLSurfaceCanvas canvas, GLSurface surface,
            ...
            synchronized (mRenderLock) {
                ...
                if (mBlit && surface.getBackingWidth() > 0) {
                    ...
                    glBindFramebuffer(GL_DRAW_FRAMEBUFFER, minecraft.getFramebuffer().fbo);
