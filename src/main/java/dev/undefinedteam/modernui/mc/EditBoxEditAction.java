...
public class EditBoxEditAction extends UndoOperation<TextFieldWidget> {
    ...
    @Override
    public void undo() {
        TextFieldWidget target = getOwnerData();
        applyUndoOrRedo(
                target,
                mStart,
    ...
    private static void applyUndoOrRedo(TextFieldWidget target,
        ...
        if (0 <= newCursorPos && newCursorPos <= text.length()) {
            target.setSelectionStart(newCursorPos);
            target.setSelectionEnd(newCursorPos);
        }
    }
