...
public class MSAA extends Framebuffer {
    public static final int MIN_SAMPLES = 2;
    public static final int MAX_SAMPLES = GL30.glGetInteger(GL30C.GL_MAX_SAMPLES);

    private static final Map<Integer, MSAA> INSTANCES = new HashMap<>();
    private static final List<MSAA> ACTIVE_INSTANCES = new ArrayList<>();
    ...
    public static MSAA getInstance(int samples) {
        return INSTANCES.computeIfAbsent(samples, x -> new MSAA(samples));
    }

    // MSAA 性能开销过大， 不建议使用，会疯狂降fps
    public static void use(Runnable drawAction) {
        if (ClientConfig.get().msaa.get()) {
    ...
    public static void use(int samples, Framebuffer mainBuffer, Runnable drawAction) {
        ...
        MSAA msaaBuffer = MSAA.getInstance(samples);
