 ...
 * @ClassName: ItemPhysics
 */
@StringEncryption
@ControlFlowObfuscation
public class ItemPhysics extends Module {
    public ItemPhysics() {
        super(Categories.Render, "item-physics", "Physical Drop");
    }
    ...
    private BakedModel getModel(ItemEntity entity) {
        ...
        // Mojang be like
        if (itemStack.isOf(Items.TRIDENT)) return mc.getItemRenderer().getModels().getModelManager().getModel(ItemRenderer.TRIDENT_IN_HAND);
        if (itemStack.isOf(Items.SPYGLASS)) return mc.getItemRenderer().getModels().getModelManager().getModel(ItemRenderer.SPYGLASS_IN_HAND);
            ...
            for (BakedQuad _quad : model.getQuads(null, face, random)) {
                ...
                for (int i = 0; i < 4; i++) {
                    switch (_quad.getFace()) {
                        case DOWN -> minY = Math.min(minY, quad.genshin$getY(i));
                        case UP -> maxY = Math.max(maxY, quad.genshin$getY(i));
                        case NORTH -> minZ = Math.min(minZ, quad.genshin$getZ(i));
                        case SOUTH -> maxZ = Math.max(maxZ, quad.genshin$getZ(i));
                        case WEST -> minX = Math.min(minX, quad.genshin$getX(i));
                        case EAST -> maxX = Math.max(maxX, quad.genshin$getX(i));
                    }
                }
            }
        }
