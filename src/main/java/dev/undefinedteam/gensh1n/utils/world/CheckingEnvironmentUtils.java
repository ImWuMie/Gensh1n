...
public class CheckingEnvironmentUtils {
    ...
    public static boolean has2BlocksOfPlaceToPlacePiston(ClientWorld world, BlockPos blockPos) {
        ...
        if (!blockState1.isAir() && blockState1.getBlock().getHardness() < 45.0f) {
            BreakUtils.simpleBreakBlock(pos1);
        }
        var blockState2 = world.getBlockState(pos1);
        if (!blockState2.isAir() && blockState2.getBlock().getHardness() < 45.0f) {
            BreakUtils.simpleBreakBlock(pos2);
        }
        // 实体碰撞箱
        boolean b = true;
